...
export class ButtonBarRenderer {
    ...
    createMainButtonsContainer() {
        ...
        mainButtons.forEach(buttonConfig => {
            if (buttonConfig.isButtonGroup && buttonConfig.dropdownButton) {
                const buttonGroup = this.createButtonGroup(buttonConfig);
    ...
    /**
     * Create a button group with dropdown
     * @param {Object} buttonConfig - Button configuration
     * @returns {HTMLElement} Button group element
     * @private
     */
    createButtonGroup(buttonConfig) {
        const buttonGroup = document.createElement('div');
        buttonGroup.className = 'spellblock-button-group';

        // Create main button
        const mainButton = this.createMainButton(buttonConfig);
        mainButton.className += ' spellblock-main-button';
        buttonGroup.appendChild(mainButton);
        this.buttonElements.set(buttonConfig.id, mainButton);

        // Create dropdown button
        if (buttonConfig.dropdownButton) {
            const dropdownButton = document.createElement('button');
            dropdownButton.id = buttonConfig.dropdownButton.id;
            dropdownButton.className = `${CSS_CLASSES.editorBtn} ${buttonConfig.dropdownButton.className}`;
            dropdownButton.setAttribute('type', 'button');
            dropdownButton.setAttribute('tabindex', '-1');
            dropdownButton.setAttribute('title', buttonConfig.dropdownButton.tooltip || 'Open dropdown');
            
            const dropdownContent = document.createElement('span');
            dropdownContent.className = CSS_CLASSES.buttonContent;
            dropdownContent.innerHTML = `
                <span class="${CSS_CLASSES.buttonIcon}">${buttonConfig.dropdownButton.icon}</span>
            `;
            
            dropdownButton.appendChild(dropdownContent);
            buttonGroup.appendChild(dropdownButton);
            this.buttonElements.set(buttonConfig.dropdownButton.id, dropdownButton);
        }
