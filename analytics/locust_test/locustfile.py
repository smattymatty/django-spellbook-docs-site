...
class WebsiteUser(HttpUser):
    ...
    def visit_trackable_page(
        ...
        self.client.get(
            ...
            name=f"{url.split('?')[0]}_tracked",
        )  # Use name for grouping in Locust UI
...
class APIUser(HttpUser):
            ...
            if response.ok:  # Checks for 2xx status codes
                try:
                    ...
                    if "id" in response_data:
                        ...
                        response.failure(
                            f"ID missing in successful POST response: {response.text}"
                        )
                except json.JSONDecodeError:
                    response.failure(
                        f"Failed to decode JSON from successful POST: {response.text}"
                    )
            else:
                response.failure(
                    f"Failed to create markdown: {response.status_code} - {response.text}"
                )
    ...
    def list_markdown(
        ...
        self.client.get(
            "/api/v1/stored-markdown/",
            name="/api/v1/stored-markdown/ (GET List)",
        )

    @task(3)
    def get_markdown_detail(
        self,
    ):
        ...
        self.client.get(
            f"/api/v1/stored-markdown/{markdown_id}/",
            name="/api/v1/stored-markdown/{id} (GET Detail)",
        )

    @task(1)
    def get_rendered_html_page(
        self,
    ):
        if not self.created_markdown_ids:
        ...
        self.client.get(
            f"/api/v1/rendered-markdown/{markdown_id}/",
            name="/api/v1/rendered-markdown/{id} (GET HTML Page)",
        )
    ...
    def delete_markdown(
        ...
        markdown_id = self.created_markdown_ids.pop(
            random.randrange(len(self.created_markdown_ids))
        )
        self.client.delete(
            f"/api/v1/stored-markdown/{markdown_id}/",
...
class SecurityTestAPIUser(HttpUser):
    ...
    def post_malformed_json(
        ...
        self.client.post(
            ...
            },  # Still claim it's JSON
            name="/api/v1/stored-markdown/ (POST Malformed JSON)",
            catch_response=True,
        )  # Important to check response status

    @task(10)
    def post_unexpected_fields(
        self,
    ):
        payload = {
    ...
    @task(10)
    def post_missing_required_fields(
        self,
    ):
    ...
    @task(15)
    def post_markdown_with_potential_xss(
        self,
    ):
    ...
    @task(5)
    def attempt_path_traversal_or_sqli_like_in_pk(
        self,
    ):
        ...
        self.client.get(
            f"/api/v1/stored-markdown/{pk_attempt}/",
    ...
    def oversized_payload(
        ...
        large_markdown = "# Large Content\n" + (
            "This is a very long line of text. " * 10000
        )  # ~2MB
        payload = {
        ...
        self.client.post(
            ...
            catch_response=True,
        )  # Catch to see if it's a 413 Payload Too Large or similar
