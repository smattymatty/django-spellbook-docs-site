...
from django.test import (
    Client,
)  # Use the test client for full request simulation
...
class PageViewMiddlewareTests(TestCase):
    def setUp(
        ...
        self.client = (
            Client()
        )  # Client simulates full requests through middleware stack
    ...
    def test_middleware_excluded_exact_path(
        self,
    ):
        ...
        self.assertEqual(
            ...
            404,
        )  # Assuming no view exists
        self.assertEqual(
            PageView.objects.count(),
            0,
        )
        self.assertEqual(
            DailyPageViewCount.objects.count(),
            0,
        )
        self.assertEqual(
            UniqueVisitor.objects.count(),
            0,
        )

    @override_settings(ANALYTICS_EXCLUDED_PATHS=["/custom/exclude/"])
    def test_middleware_excluded_exact_path_from_settings(
        self,
    ):
    ...
    def test_middleware_excluded_prefix_path(
        self,
    ):
        ...
        self.assertEqual(
            response.status_code,
            404,
        )  # Assuming no admin view is set up
        self.assertEqual(
            PageView.objects.count(),
            0,
        )
    ...
    def test_middleware_excluded_prefix_path_from_settings(
        self,
    ):
    ...
    def test_middleware_excluded_authenticated_user(
        self,
    ):
    ...
    def test_middleware_excluded_ajax_request(
        self,
    ):
    ...
    @patch(
        "analytics.utils.validate_url",
        return_value=False,
    )
    def test_middleware_excluded_invalid_url(
        self,
        mock_validate_url,
    ):
    ...
    def test_middleware_tracks_normal_request(
        self,
    ):
        ...
        self.assertEqual(
            response.status_code,
            404,
        )  # No view needed for middleware test
    ...
    def test_middleware_updates_counts_on_repeat_visit(
        self,
    ):
        ...
        dpc = DailyPageViewCount.objects.get(
            url=TRACKABLE_PATH,
            date=date.today(),
        )
        self.assertEqual(
        ...
        self.assertTrue(
            timezone.now() - uv.last_visit < timedelta(seconds=5)
        )  # last_visit updated

    @override_settings(ANALYTICS_TRACK_AUTHENTICATED_USERS=True)
    def test_middleware_tracks_authenticated_user_if_enabled(
        self,
    ):
    ...
    def test_middleware_tracks_ajax_if_enabled(
        self,
    ):
    ...
    def test_ip_extraction_x_forwarded_for(
        self,
    ):
        ...
        response = self.client.get(
            TRACKABLE_PATH,
            HTTP_X_FORWARDED_FOR=f"{ip}, 192.168.1.1",
        )
        self.assertEqual(
            PageView.objects.count(),
            1,
    ...
    def test_ip_extraction_remote_addr(
        self,
    ):
    ...
    def test_ip_invalid_ip_stored_as_default(
        self,
    ):
        """Test that invalid IPs are stored as 0.0.0.0"""
        response = self.client.get(
            TRACKABLE_PATH,
            REMOTE_ADDR="INVALID_IP_STRING",
        )
        self.assertEqual(
            PageView.objects.count(),
            1,
    ...
    def test_middleware_bot_detection(
        self,
    ):
        """Check if bot detection in UniqueVisitor works via middleware"""
        response = self.client.get(
            TRACKABLE_PATH,
            HTTP_USER_AGENT="Googlebot/2.1 Test",
        )
        self.assertEqual(
            UniqueVisitor.objects.count(),
            1,
...
@override_settings(
    ANALYTICS_VALIDATE_URL=True
)  # Ensure validation is active for this class
class PageViewMiddlewareValidateUrlIntegrationTests(TestCase):
    ...
    def test_integration_skips_forbidden_substrings(
        self,
    ):
        """
        ...
        for url in invalid_urls:
            with self.subTest(url=url):
                ...
                self.assertEqual(
                    PageView.objects.count(),
                    0,
                    f"PageView created for substring URL: {url}",
                )
                self.assertEqual(
                    DailyPageViewCount.objects.count(),
                    0,
                    f"DailyCount created for substring URL: {url}",
                )
                self.assertEqual(
                    UniqueVisitor.objects.count(),
                    0,
                    f"UniqueVisitor created for substring URL: {url}",
                )

    def test_integration_skips_forbidden_extensions(
        self,
    ):
        """
        ...
        for url in invalid_urls:
            with self.subTest(url=url):
                ...
                self.assertEqual(
                    PageView.objects.count(),
                    0,
                    f"PageView created for extension URL: {url}",
                )
                self.assertEqual(
                    DailyPageViewCount.objects.count(),
                    0,
                    f"DailyCount created for extension URL: {url}",
                )
                self.assertEqual(
                    UniqueVisitor.objects.count(),
                    0,
                    f"UniqueVisitor created for extension URL: {url}",
                )

    def test_integration_skips_long_urls(
        self,
    ):
        """
        ...
        long_url = (
            "/" + "a" * MAX_URL_LENGTH
        )  # This URL has length MAX_URL_LENGTH + 1
        ...
        self.assertEqual(
            PageView.objects.count(),
            0,
            f"PageView created for long URL",
        )
        self.assertEqual(
            DailyPageViewCount.objects.count(),
            0,
            f"DailyCount created for long URL",
        )
        self.assertEqual(
            UniqueVisitor.objects.count(),
            0,
            f"UniqueVisitor created for long URL",
        )

    def test_integration_tracks_valid_urls_when_validation_active(
        self,
    ):
        """
        ...
        self.assertEqual(
            PageView.objects.count(),
            1,
            f"PageView not created for valid URL",
        )
        self.assertEqual(
            DailyPageViewCount.objects.count(),
            1,
            f"DailyCount not created for valid URL",
        )
        self.assertEqual(
            UniqueVisitor.objects.count(),
            1,
            f"UniqueVisitor not created for valid URL",
        )
