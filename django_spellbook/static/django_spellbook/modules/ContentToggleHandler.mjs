...
export class ContentToggleHandler {
  ...
  showContent() {
    ...
    if (this.toggleType === "hidden") {
      this.contentElement.classList.forEach((className) => {
  ...
  hideContent() {
    ...
    if (this.toggleType === "hidden") {
      setTimeout(() => {
        // Add the specific hidden class used (e.g., 'sb-hidden' or 'hidden')
        this.contentElement.classList.add(this.getHiddenClass());
      }, this.animationDelay);
    }
  }
  ...
  toggleContent() {
    ...
    const isContentVisible =
      this.toggleType === "hidden"
        ? !Array.from(this.contentElement.classList).some((className) =>
  ...
  getHiddenClass() {
    // First, check if there's a data attribute specifying the hidden class
    const specifiedHiddenClass = this.container.dataset.hiddenClass;
    if (specifiedHiddenClass) {
      return specifiedHiddenClass;
    }

    // If not specified, look for an existing class containing 'hidden'
    const existingHiddenClass = Array.from(this.contentElement.classList).find(
      (className) => className.includes("hidden")
    );
    if (existingHiddenClass) {
      return existingHiddenClass;
    }

    // If no existing 'hidden' class found, default to 'hidden'
    return "hidden";
  }
}
