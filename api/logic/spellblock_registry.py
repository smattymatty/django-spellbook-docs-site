...
Provides clean, testable helper functions for extracting
and processing spellblock information.
"""
...
def extract_kwargs_parameters(block_class) -> Dict[str, Dict[str, Any]]:
    """
    Extract parameters from self.kwargs.get() patterns in the
    get_context method.
    ...
    try:
        ...
        for param_name, raw_default in matches:
            ...
            default_value = (
                raw_default.strip().strip("'\"") if raw_default else ""
            )

            # Infer parameter type from default value
            param_type, processed_default = _infer_parameter_type(
                default_value
            )
...
def build_spellblock_registry() -> List[Dict[str, Any]]:
    """
    Build the complete spellblock registry with all block information
    and parameters.
...
def _get_block_description(block_class, block_name: str) -> str:
    """
    Get the description for a spellblock from its docstring
    ...
    """
    return getattr(block_class, "__doc__", None) or f"SpellBlock: {block_name}"
