...
Provides clean, testable helper functions for rendering
and sanitizing markdown content.
"""
...
def sanitize_html_content(html_content: str, strict_mode: bool = False) -> str:
    ...
    try:
        ...
        logger.debug(
            f"Successfully sanitized HTML ({len(html_content)} chars -> "
            f"{len(sanitized_html)} chars)"
        )
...
def process_markdown_preview(
    ...
    try:
        ...
        if enable_sanitization:
            html_output = sanitize_html_content(
                html_output, strict_mode=strict_mode
            )
        else:
            logger.warning(
                "HTML sanitization is DISABLED - this may be a security risk"
            )
        ...
        logger.error(
            f"Failed to process markdown preview: {type(e).__name__} - {e}"
        )
        error_html = (
            f"<div class='error'>"
            f"Error processing markdown: {escape(str(e))}</div>"
        )
