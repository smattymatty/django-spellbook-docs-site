"""
Tests for markdown preview helper functions.
"""
...
class TestRenderMarkdownToHtml(TestCase):
    ...
    def test_render_markdown_exception_fallback(self, mock_logger, mock_render):
...
class TestSanitizeHtmlContent(TestCase):
    ...
    def test_sanitize_html_preserves_safe_attributes(self):
        ...
        result = sanitize_html_content(html)
        self.assertEqual(result, '<a href="https://example.com" title="Example">Link</a>')
...
class TestProcessMarkdownPreview(TestCase):
    ...
    def test_process_markdown_success_with_sanitization(self, mock_sanitize, mock_render):
        """Test successful markdown processing with sanitization."""
        mock_render.return_value = "<p>Hello World</p>"
        mock_sanitize.return_value = "<p>Hello World</p>"
        
        response = process_markdown_preview("# Hello World", enable_sanitization=True)
        ...
        mock_render.assert_called_once_with("# Hello World")
        mock_sanitize.assert_called_once_with("<p>Hello World</p>", strict_mode=False)
    
    @patch('api.logic.markdown_preview.render_markdown_to_html')
    @patch('api.logic.markdown_preview.sanitize_html_content')
    @patch('api.logic.markdown_preview.logger')
    def test_process_markdown_success_without_sanitization(self, mock_logger, mock_sanitize, mock_render):
        """Test successful markdown processing without sanitization."""
        mock_render.return_value = "<p>Hello World</p>"
        
        response = process_markdown_preview("# Hello World", enable_sanitization=False)
    ...
    @patch('api.logic.markdown_preview.render_markdown_to_html')
    @patch('api.logic.markdown_preview.logger')
    def test_process_markdown_exception_handling(self, mock_logger, mock_render):
...
class TestIntegration(TestCase):
    ...
    def test_full_pipeline_spellblock_rendering(self):
        ...
        self.assertTrue(any(tag in content for tag in ["<h1>", "<p>", "<strong>"]))
