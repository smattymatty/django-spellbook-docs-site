...
from rest_framework.test import (
    APITestCase,
)  # Good for testing DRF views
from django.conf import (
    settings,
)  # To access MAX_API_REQUEST_SIZE
import json

from ..models import (
    StoredMarkdown,
)  # Or from api.models import StoredMarkdown
...
class StoredMarkdownAPITests(APITestCase):
    def setUp(
        ...
        self.list_create_url = reverse(
            "storedmarkdown-list-create"
        )  # From your api/urls.py name
        self.test_data_1 = {
        ...
        self.detail_url_1 = reverse(
            "storedmarkdown-detail",
            kwargs={"pk": self.stored_item_1.pk},
        )
        self.rendered_url_1 = reverse(
            "renderedmarkdown-detail",
            kwargs={"pk": self.stored_item_1.pk},
        )

    def test_create_stored_markdown_valid(
        self,
    ):
        """
        ...
        self.assertEqual(
            StoredMarkdown.objects.count(),
            2,
        )  # One from setUp, one created
        created_item = StoredMarkdown.objects.get(
            title=self.test_data_1["title"]
        )
        self.assertEqual(
            created_item.markdown_content,
            self.test_data_1["markdown_content"],
        )
        self.assertIn(
            "<h1",
            created_item.html_content,
        )  # Check if HTML was generated

    def test_create_stored_markdown_invalid_missing_content(
        self,
    ):
        """
    ...
    def test_list_stored_markdown(
        ...
        self.assertEqual(
            len(response.data),
            2,
        )  # Check based on items created
        self.assertEqual(
            response.data[0]["title"],
            "Another Item",
        )  # Assuming default ordering by -updated_at
        self.assertEqual(
            response.data[1]["title"],
            self.stored_item_1.title,
        )

    def test_retrieve_stored_markdown_detail(
        self,
    ):
        """
        ...
        self.assertIn(
            "api_url",
            response.data,
        )  # If using HyperlinkedModelSerializer
        self.assertIn(
            "rendered_html_page_url",
            response.data,
        )

    def test_retrieve_stored_markdown_detail_not_found(
        self,
    ):
        """
    ...
    def test_update_stored_markdown(
        self,
    ):
        """
        ...
        self.assertEqual(
            self.stored_item_1.markdown_content,
            updated_data["markdown_content"],
        )
        self.assertIn(
            "<h2",
            self.stored_item_1.html_content,
        )  # Check if HTML was re-generated

    def test_delete_stored_markdown(
        self,
    ):
        """
    ...
    def test_get_rendered_html_page(
        self,
    ):
        """
        ...
        response = self.client.get(
            self.rendered_url_1
        )  # No format='json' as it returns HTML
        self.assertEqual(
            response.status_code,
            status.HTTP_200_OK,
        )
        ...
        self.assertContains(
            ...
            self.stored_item_1.html_content,
            html=True,
        )  # Or more specifically if html_content is just a fragment:
        # self.assertContains(response, "<h2 id=\"existing\">Existing</h2>")
        # Check for elements from your api/sb_base.html template
        self.assertContains(
            response,
            '<link rel="stylesheet" href="/static/django_spellbook/root.css">\n<link rel="stylesheet" href="/static/django_spellbook/utilities.css">\n<link rel="stylesheet" href="/static/django_spellbook/styles.css">',
        )  # Example from a base template

    def test_post_payload_too_large(
        self,
    ):
        """
        ...
        test_data_upload_max_memory_size = (
            100  # bytes for Django's internal check
        )

        with self.settings(
            MAX_API_REQUEST_SIZE=test_max_api_size,
            ...
            markdown_chars_to_exceed_limit = (
                test_max_api_size  # Make it clearly exceed
            )
            ...
            expected_payload_size_bytes = len(
                serialized_payload.encode("utf-8")
            )

            print(f"\n[TEST_INFO] Test: test_post_payload_too_large")
            print(
                f"[TEST_INFO] Overridden MAX_API_REQUEST_SIZE (for view check): {test_max_api_size} bytes"
            )
            print(
                f"[TEST_INFO] Overridden DATA_UPLOAD_MAX_MEMORY_SIZE (for Django check): {test_data_upload_max_memory_size} bytes"
            )
            print(
                f"[TEST_INFO] Expected serialized payload size: {expected_payload_size_bytes} bytes"
            )
            ...
            actual_content_length_header = request_environ.get(
                "CONTENT_LENGTH",
                "Not set by test client",
            )

            print(f"[TEST_INFO] Response Status Code: {response.status_code}")
            print(
                f"[TEST_INFO] Actual CONTENT_LENGTH header received by WSGI app (from response.request): {actual_content_length_header}"
            )
    ...
    def test_markdown_content_sanitization_xss(
        self,
    ):
        """
        ...
        self.assertNotIn(
            "<script>",
            created_item.html_content.lower(),
        )
        self.assertNotIn(
            "<p>alert('xss')</p>",
            created_item.html_content.lower(),
        )  # Check content also, just in case
        self.assertIn(
            "<h1",
            created_item.html_content,
        )  # Ensure legitimate markdown is still processed
